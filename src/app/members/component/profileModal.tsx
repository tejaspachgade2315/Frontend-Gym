...
export default function ProfileModal({ isOpen, onClose, userId }: ProfileModalProps) {
  ...
  const fetchuserData = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/user/${userId}`, {
        ...
        <DialogContent className="w-full max-w-sm sm:max-w-lg md:max-w-xl lg:max-w-2xl mx-auto p-6 sm:p-8 md:p-10 overflow-auto bg-white shadow-lg rounded-lg">
          <div className="flex flex-col items-center space-y-4">
              ...
              src={
                userData?.image
                  ...
                  : userData?.gender === "male"
                    ? "https://www.shareicon.net/data/512x512/2016/07/21/799323_user_512x512.png"
            ...
            <div className="bg-gray-100 p-4 rounded-lg mt-6 w-full">
              <h3 className="text-lg font-medium text-gray-700">Membership Details</h3>
              <p>
              ...
              <p>
                <strong>Activity Count:</strong> {userData?.activityCount}
              </p>
            </div>
          </div>
        </DialogContent>
